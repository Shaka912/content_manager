...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    if (tags?.length > 0 && subreddit === null) {
      const post = await Post.find({  userid:userid,tags: { $in: tags } });
      if (post.length > 0 ) {
        res.json(post);
      } else {
        ...
        const subs = await Post.find({ userid:userid,status: { $in: subreddit } });
        console.log(subs)
        if( subs.length >0){
            res.json(subs);
        }else {
        ...
        const posts = await Post.find({userid:userid,"status":{$size:0}});
        if(posts.length>0){
            res.json(posts);
        }else{
